...
export class Inventory extends Component {
  ...
  detailCheck(item) {
    ...
    // localStorage.setItem("invenroryid", item.inventoryId);
    // axios
    //   .get(
  ...
  render() {
    ...
    if(inventory.length == 0) {
      inventory_table = (
        <center>
        <p style={{fontSize : '18px'}}>There is no data Yet.</p>
        </center>
        )
    }
    else {
                    ...
                    inventory.map(function(item, id) {
                      return (
                        <tr key={id}>
                          ...
                          <td scope="row">{item.sgstAmounts || "0"}</td>
                          <td scope="row">{item.cgstAmount || "0"}</td>
                          <td scope="row">{item.igstAmount || "0"}</td>
                          <td scope="row">{item.gstAmount || "0"}</td>
                          <td scope="row">{item.subTotal || "0"}</td>
                          <td scope="row">{item.total || "0"}</td>
                          <td>
                            <span
